...
export async function POST(req: NextRequest) {
  ...
  // RBAC: Only owner/admin/developer can share secrets
  const { getUserProjectRole } = await import("@/lib/permissions");
  const role = await getUserProjectRole(auth.userId, secret.projectId);
  if (!role || role === "viewer") {
    return NextResponse.json({ error: "Viewers do not have permission to share secrets" }, { status: 403 });
  }
...
export async function GET(req: NextRequest) {
  ...
  // Atomic view count check + increment to prevent race conditions
  if (share.maxViews !== null) {
    const updated = await prisma.secretShare.updateMany({
      where: {
        id: share.id,
